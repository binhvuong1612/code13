function list_request_adoption(){
    ...
    //đưa dữ liệu vào id="container" trong
        ...
        firebase.database().ref("Pet/").once('value', function(snapshot) {
            ...
            for(var k, k = 0; k < task_array_pet.length; k++){
               
                task_pet_name = task_array_pet[k][4];   

                name = task_pet_name;

                firebase.database().ref("Request_Adoption/"+name).once('value', function(snapshot) {
                  snapshot.forEach(function(childSnapshot) {
                    var childKey = childSnapshot.key;
                    var childData = childSnapshot.val();
                    task_array.push(Object.values(childData));
                        ...
                        for(var i, i = 0; i < task_array.length+2; i++){
                            ...
                            alert("Thong tin:"+task_address_adopter+":"+task_email_adopter+":"+task_name_adopter+":"+task_pet_adopt+":"+task_phone_adopter)
