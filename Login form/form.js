        ...
        promise.catch(e => alert(e.message));
                  ...
                  if (user) {
                    ...
                    // check_type_user();
                        ...
                        task_array_NV = [];//khai bao bien

                        firebase.database().ref("Employees").once('value', function(snapshot) {
                          snapshot.forEach(function(childSnapshot) {
                            var childKey = childSnapshot.key;
                            var childData = childSnapshot.val();
                            task_array_NV.push(Object.values(childData));
                          });
                          //tạo vòng lặp để tạo các task 
                          for(var i, i = 0; i < task_array_NV.length; i++){
                            task_address = task_array_NV[i][0];
                            task_birthday = task_array_NV[i][1];
                            task_email = task_array_NV[i][2];
                            task_key = task_array_NV[i][3];
                            task_password = task_array_NV[i][4];
                            task_phone = task_array_NV[i][5];
                            task_type_user = task_array_NV[i][6];
                            task_username = task_array_NV[i][7];


                            if (email == task_email) {
                              check_type_user = "employees";
                              alert("Open Display Employees");
                              window.location= "file:///C:/Users/ADMIN/Desktop/newupdate/NHAN_VIEN/admin.html";
                              break;
                            }
